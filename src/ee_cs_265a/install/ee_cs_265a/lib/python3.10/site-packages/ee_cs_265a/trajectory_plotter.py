...
class TrajectoryPlotter(Node):
    ...
    def save_plot(self):
        ...
        outer = plt.Rectangle((-15, -9), 30, 18, fill=False, edgecolor='black', linewidth=2, label='Outer wall')
        island_ul = plt.Rectangle((-11, 1.5), 9.5, 4, fill=True, facecolor='lightgray', edgecolor='black', linewidth=1.5, label='Upper-left island')
        island_lr = plt.Rectangle((1.5, -5.5), 9.5, 4, fill=True, facecolor='lightgray', edgecolor='black', linewidth=1.5, label='Lower-right island')
        ax.add_patch(outer)
        ax.add_patch(island_ul)
        ax.add_patch(island_lr)
